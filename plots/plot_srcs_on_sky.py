...
Modified further to grab source names and data from a text file
...
o.add_option('--srcfile',type=str,
    help='text file with srcnames and data,use # for comments')
o.add_option('--datacol',type=int,
    help='which column of data to plot (assumes space delimited, zero index, srcname in col0)')
o.add_option('--axcolor',default='w',
    help='the color of the axis')
opts,args = o.parse_args(sys.argv[1:])

#parse the file
if not opts.srcfile is None:
...
#        if opts.osys=='ga':slons *= -1
    ...
    map.drawparallels(n.arange(-90,90,30)[1:],)
    ...
    ax = p.subplot(m1,m2,i+1,axisbg=opts.axcolor)
...
#        print M.shape,mask.shape
        ...
        DC = map.contourf(x,y,D,levels,lw=6,cmap=p.get_cmap('Blues'))
    ...
    if opts.osys=='ga': 
        ...
        data = n.fliplr(data)
        print data.shape
    if not opts.skip_im:map.imshow(data, vmax=max, vmin=min, cmap=cmap,interpolation=opts.interp)
    ...
    if not opts.src is None:
        ...
        for name, xpt, ypt, flx in zip(snams, sx, sy, sflxs):
            ...
            if not opts.datacol is None:
                sd = float(srcdata[name])
#                if n.abs(sd)>0:
                marker = 'x'
                ...
                if sd!=0:
                    ...
                    sizes[size] = name
                    if not opts.highquality is None:
                        size *= opts.highquality
                    #if not opts.highquality is None:
                    #    size =(1./n.abs((sd - srcdatamin)/srcdatamax)+5)*opts.highquality
                    #else:
...
#        C = map.contour(X,Y,n.log10(W))
    ...
    tight_layout()
#    if not opts.datacol is None:
#        inset = axes([0.35,0.5,0.3,.1],frameon=False)
#        inset.set_xticks([])
#        inset.set_yticks([])
#        #inset.set_axes_bgcolor([])
#        print 'len(sizes)',len(sizes.keys())
#        s = (n.min(sizes.keys()),imedian(sizes.keys()),n.max(sizes.keys()))
#        inset.scatter([1,2,3],y=[1,1,1],s=s,marker='x',color='k')
#        inset.set_ylim([-2,2])
#        inset.set_xlim([0,4])
#        for i,size in enumerate(s):
#            inset.annotate(sizes[size],[i+1,-1])
#            print size,sizes[size]
#        #inset.annotate([sizes[l] for l in s],[1,2,3],s,xytext=[-10,0],textcoords='offset points')
