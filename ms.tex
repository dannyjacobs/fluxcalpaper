...
1.4\%, confirming that Pictor A is a linear power law through the EoR band, and is an excellent calibrator. 
...
\cite{Bernardi:2012p9020,Sullivan:2012p9457,Morales:2012p8790} a process which, to be affective, must be done to better than 1\%
...
\S\ref{sec:results}). Smaller indicates better agreement with past data.  Orange dots indicate sources with no model overlap at 76\% \ref{fig:SI_contour_new}. 
\label{fig:error_map}
}
\end{figure*}
%
...
the target location and summing to produce a spectral time series of perceived flux $S_p$. The spectrum is then isolated
from neighboring sources by filtering and averaged in the time domain.  This time average is 
weighted by a model of PAPER's primary beam response ($B$).  
\begin{align}
...
The pre factor in Eq \ref{eq:g} has been integrated in time over the full azimuthal sweep across the sky and therefor depends only on declination. The rate at which this factor
...
have all been set, as best as possible, to the Baars et al scale to which all other points are tied \citep{Baars:1977p9678}. Using a Bayesian analysis we compute the 
...
Measurements of ambient temperature versus time on the balun amplifier of a fiducial antenna and near
...
comes after time and frequency averaging in sections \ref{sec:Calibration} \ref{sec:flux_scale}.   
